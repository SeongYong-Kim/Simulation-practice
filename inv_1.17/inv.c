...
float area_holding, area_shortage, holding_cost, incremental_cost, maxlag,
      ...
      total_ordering_cost, total_shortage_cost, short_time, short_num;
FILE  *infile, *outfile;
...
int main()  /* Main function. */
{
    ...
    fprintf(outfile, "        Average        Average              Average                  Average\n");
    fprintf(outfile, "  Policy       total cost    ordering cost");
    fprintf(outfile, "  holding cost   shortage cost");
    fprintf(outfile, "    proportion backlog time");
    fprintf(outfile, "    num express orders");
...
void report(void)  /* Report generator function. */
{
    ...
    float avg_holding_cost, avg_ordering_cost, avg_shortage_cost, prop_short_time;

    avg_ordering_cost = total_ordering_cost / num_months;
    avg_holding_cost  = holding_cost * area_holding / num_months;
    avg_shortage_cost = total_shortage_cost / num_months;
    prop_short_time = short_time / num_months;
    fprintf(outfile, "\n\n(%3d,%3d)%15.2f%15.2f%15.2f%15.2f%19.2f%26.2f",
            smalls, bigs,
            avg_ordering_cost + avg_holding_cost + avg_shortage_cost,
            avg_ordering_cost, avg_holding_cost, avg_shortage_cost, prop_short_time, short_num);
                                     ...
                                     statistics. */
{
    ...
    if (inv_level < 0) {
        ...
        /*새로운 shortage cost 계산*/
        total_shortage_cost += -(inv_level) * shortage_cost + 48;
        /*shortage_time 계산*/
        short_time += time_since_last_event;
        /*shortage 횟수 계산*/
        ++short_num;
    }
    else if (inv_level > 0)
